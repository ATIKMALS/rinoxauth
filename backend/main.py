...
def _make_license_key() -> str:
    """Generate 128-bit license key (32 hex chars)"""
    token = secrets.token_hex(16).upper()  # 32 hex characters (128-bit)
...
def _init_db() -> None:
    ...
    try:
        ...
        print(f"Warning: User migration: {e}")
    ...
    try:
        ...
        print(f"Warning: License migration: {e}")
    ...
    try:
        ...
        print(f"Warning: API key migration: {e}")
    ...
    try:
        ...
        print(f"Warning: Applications migration: {e}")
    ...
    try:
        ...
        print(f"Warning: Reseller migration: {e}")
    ...
    print(f"   Warning: No default users/licenses - Register via /api/auth/register-admin")       
...
def sdk_auth(payload: AuthPayload):
    ...
    if req_type == "license-login":
        ...
        # ✅ Check if license has linked user
        if lic["user_id"]:
            user = conn.execute(
                "SELECT * FROM users WHERE id=? AND application_id = ? AND is_active=1",
                (lic["user_id"], app["id"])
            ).fetchone()

            if not user:
                conn.close()
                return _error("Linked user not found or inactive", 404)

            conn.execute("UPDATE users SET last_login=? WHERE id=?", (_now_iso(), user["id"]))
            _log_activity(conn, "login", "info", f"License login: {user['username']}", user["id"])
            conn.commit()
            conn.close()
        ...
        # ❌ No user linked - must register first
        conn.close()
        return _error("No user linked to this license. Register first.", 404)
...
def web_login(payload: LoginPayload):
    ...
    # Require app_id for security and consistency
    if not payload.app_id:
        conn.close()
        return _error("Application ID required", 400)
...
def oauth_session(payload: OAuthSessionPayload):
    ...
    if not user:
        conn.close()
        return _error("User account not found. Please register first or contact admin.", 404)
...
def get_dashboard_stats(request: Request):
    """Get dashboard statistics - filtered by application from header"""
    conn = _db()
    
    # Get app_id from header (set by middleware)
    app_id = request.headers.get("X-Application-Id")
    if not app_id:
        conn.close()
        return _error("Application ID required", 400)
    ...
    try:
        ...
        conn.close()
        return _error("Invalid Application ID", 400)
    ...
    failed_logins = conn.execute(
        ...
        AND datetime(al.created_at) >= datetime(?, '-1 day')
        AND (u.application_id = ? OR al.user_id IS NULL)
        """,
        (_now_iso(), app_id),
    ).fetchone()["c"]
...
@app.get("/api/users")
def get_users(request: Request, hide_auto: bool = False):
    """Get users - filtered by application from header"""
    conn = _db()
    ...
    # ✅ Filter out auto-generated users (username starting with "user_")
    if hide_auto:
        rows = [r for r in rows if not r["username"].startswith("user_")]
...
def get_licenses(request: Request):
    """Get all licenses - filtered by application from header"""
    conn = _db()
...
def get_app_credentials(app_id: str):
    ...
    return _success("ok", {
        ...
        "version": row["version"],
        "client_portal": "https://discerning-kindness-production-8967.up.railway.app",
    })
...
def get_analytics(request: Request):
    """Get analytics data - filtered by application from header"""
    conn = _db()
...
def get_activity_logs(request: Request):
    """Get activity logs - filtered by application from header"""
    conn = _db()
    ...
    rows = conn.execute(
        """
        SELECT DISTINCT al.* FROM activity_logs al
        LEFT JOIN users u ON u.id = al.user_id
        WHERE u.application_id = ? OR al.user_id IS NULL
...
if __name__ == "__main__":
    ...
    print(f"   Warning: No default data - First user must register")
    print("=" * 60)
